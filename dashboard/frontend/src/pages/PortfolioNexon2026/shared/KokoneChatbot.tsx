/**
 * 코코네 포트폴리오 안내 챗봇 (사이오닉 챗봇 포크 — 핑크 테마).
 * 플로팅 캐릭터(GIF) 클릭 → 챗 패널. /api/portfolio/ask/sionic/stream SSE 재사용
 * (백엔드 페르소나는 "양건호 포트폴리오 안내"로 회사 무관하게 동작).
 */
...
const CHIPS = ['양건호가 어떤 사람이야?', '캐릭터 LoRA 만든 거 알려줘', 'AI/ML 경험 알려줘', 'kasaterm 하네스가 뭐야?']
...
export default function KokoneChatbot() {
  const [open, setOpen] = useState(false)
  const [msgs, setMsgs] = useState<Msg[]>([
    { role: 'bot', text: '안녕하세요 ✦ 양건호 포트폴리오에 대해 뭐든 물어보세요!' },
  ])
  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      setMsgs((m) => {
        const c = [...m]
        c[c.length - 1] = { role: 'bot', text: '앗, 잠깐 문제가 생겼어요 ✦ 다시 물어봐 주세요!' }
        return c
      })
    } finally {
      setLoading(false)
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            style={{ position: 'fixed', bottom: 24, right: 24, zIndex: 151, width: 'min(380px, calc(100vw - 32px))', height: 'min(560px, calc(100vh - 90px))', background: '#fff', borderRadius: 20, boxShadow: '0 24px 70px rgba(26,6,20,0.3)', display: 'flex', flexDirection: 'column', overflow: 'hidden', border: `1px solid ${ACCENT}22` }}
          >
            {/* 헤더 */}
            <div style={{ background: `linear-gradient(135deg, ${ACCENT}, #FF6FB0)`, padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontFamily: FONT, fontSize: 11.5, color: 'rgba(255,255,255,0.85)' }}>양건호 포트폴리오를 안내해요 ✦</div>
              </div>
            ...
            {/* 메시지 */}
            <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: 18, display: 'flex', flexDirection: 'column', gap: 12, background: '#FFF5FA' }}>
              {msgs.map((m, i) => (
                <div key={i} style={{ alignSelf: m.role === 'user' ? 'flex-end' : 'flex-start', maxWidth: '82%' }}>
                  <div style={{ fontFamily: FONT, fontSize: 14, lineHeight: 1.6, padding: '10px 14px', borderRadius: 14, wordBreak: 'keep-all', whiteSpace: 'pre-wrap', ...(m.role === 'user' ? { background: ACCENT, color: '#fff', borderBottomRightRadius: 4 } : { background: '#fff', color: '#2A0E22', border: '1px solid rgba(26,6,20,0.07)', borderBottomLeftRadius: 4 }) }}>
            ...
            {/* 입력 */}
            <div style={{ padding: '12px 14px', borderTop: '1px solid rgba(26,6,20,0.08)', display: 'flex', gap: 8, background: '#fff' }}>
              <input
                value={input}
                ...
                disabled={loading}
                style={{ flex: 1, fontFamily: FONT, fontSize: 14, padding: '10px 14px', borderRadius: 9999, border: '1px solid rgba(26,6,20,0.14)', outline: 'none' }}
              />
