...
code([
    '#@title 6. train.json 가져오기\n',
    'import shutil, json\n',
    '\n',
    '# Drive에서 train.json 찾기\n',
    'search_paths = [\n',
    '    "/content/drive/MyDrive/train.json",\n',
    '    f"/content/drive/MyDrive/{CHARACTER}/train.json",\n',
    '    f"{DRIVE_BACKUP}/train.json",\n',
    ']\n',
    '\n',
    'found = False\n',
    ...
    '\n',
    'if not found:\n',
    '    print("[!] train.json을 Google Drive 루트에 업로드하세요")\n',
    'else:\n',
    '    with open(f"{DATA_DIR}/train.json") as f:\n',
    '        data = json.load(f)\n',
    '    print(f"데이터: {len(data)}개")\n',
    '    print(f"포맷: {list(data[0].keys())}")\n',
    '    print(f"샘플: {json.dumps(data[0], ensure_ascii=False)[:200]}")\n',
])

# === 7. dataset_info.json ===
code([
    '#@title 7. dataset_info.json 생성\n',
    'import json\n',
    '\n',
    '# OpenAI 스타일: messages > role/content\n',
    'dataset_info = {\n',
    '    CHARACTER: {\n',
...
code([
    ...
    '})\n',
    '\n',
    'for q in ["안녕!", "너 누구야?", "10킬 했어!", "양궁장이네", "계속 져..."]:\n',
    '    r = chat_model.chat([{"role": "user", "content": q}])\n',
    '    print(f"Q: {q}")\n',
    '    print(f"A: {r}\\n")\n',
])
...
with open("Qwen25_Omni_Finetune_A100.ipynb", "w", encoding="utf-8") as f:
    json.dump(nb, f, ensure_ascii=False, indent=1)

print(f"노트북 생성: {len(nb['cells'])}셀")
