...
export const STATS = [
  ...
  { label: '메이플 라이브 플레이', value: '10', unit: '년 7개월', sub: '직업 월드 랭킹 989위' },
  { label: '디자인 공모전', value: '1', unit: '건 입상', sub: '커뮤니케이션 국제 디자인 공모전' },
] as const

export const ABOUT = `데비&마를렌은 1인이 9개월간 158개 Discord 서버에 운영 중인 라이브 LLM 챗봇입니다. 단순 명령어 봇이 아니라 사용자 응대·정책 수립·이상 호출 차단·데이터 분석·인프라 자동화까지 라이브 서비스의 운영 사이드를 모두 직접 다뤘습니다. 메이플스토리 약 10년 7개월의 헤비유저 시점과, 1인 봇 운영자의 라이브 서비스 시점이 결합된 자리에서 게임서비스 직무의 본질 — 사용자 한 명의 문제를 데이터와 정책으로 해결해 콘텐츠가 떠받쳐지는 환경을 만드는 일 — 을 가장 잘 수행할 수 있다고 자신합니다.`
...
export const JD_MATCHES = [
  ...
  },
  {
    ...
    evidence: [
      ...
      '메이플스토리 약 10년 7개월 헤비유저 시점에서 라이브 서비스가 패치·이벤트·신규 직업·시즌 콘텐츠로 어떻게 사용자 잔존율을 만드는지 사용자 입장에서 추적해 옴',
...
export const ELIGIBILITY = {
  ...
  body:
    '게임서비스 직무 이해도 / 새로운 게임 적극 학습 / 논리 글쓰기 / 문제 해결 / 협업 / MS-OFFICE 모두 충족. 라이브 봇 운영 9개월 + 메이플 헤비유저 10년 7개월의 사용자/운영자 양쪽 시점.',
} as const
