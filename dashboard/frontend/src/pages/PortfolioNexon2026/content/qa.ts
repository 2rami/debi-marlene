...
export const JD_MATCHES = [
  ...
  },
  {
    ...
    evidence: [
      '메이플스토리 약 10년 7개월 / Lv.284 / 직업 월드 랭킹 989위 / 제네시스 22성 해방 — 라이트 유저가 절대 안 밟는 직업·해방·유니온 격자 깊이의 엣지 케이스를 일상적으로 밟는 헤비 유저',
...
export const ELIGIBILITY = {
  ...
  body:
    '2014년 10월부터 약 10년 7개월간 메이플스토리를 플레이하며, 검은 마법사 라이프타임 보스와 그란디스 어센틱 지역까지 거의 모든 라이프타임 콘텐츠를 직접 거쳤습니다. 단순 만렙이 아니라 직업 시너지·해방 퀘스트·유니온 격자 같은 시스템 깊이까지 사용자 입장에서 종합적으로 이해하며, 라이트 유저나 외주 QA가 못 잡는 깊이의 결함을 우선 의심·재현할 수 있습니다.',
} as const
...
export const GAMES = [
  {
    ...
    detail: '오로라 / 아크메이지(썬,콜) Lv.284 / 직업 월드 랭킹 989위 / 제네시스 22성 해방 — 헤비 유저',
  },
