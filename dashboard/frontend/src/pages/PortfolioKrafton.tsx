...
const SECTIONS = [
  ...
  { id: 'ai-workflow', label: '하네스 엔지니어링' },
      ...
      <section id="ai-workflow" className="py-24 relative">
        <div className="relative max-w-5xl mx-auto px-6">
          <div className="text-center mb-16">
            ...
            <FadeIn delay={0.1}>
              <p className={`text-lg max-w-2xl mx-auto mt-4 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                프롬프트 엔지니어링이 모델에게 &lsquo;무엇을 말할지&rsquo;를 설계한다면,<br />
                하네스 엔지니어링은 모델이 동작하는 &lsquo;전체 운영 환경&rsquo;을 설계합니다.
              </p>
            </FadeIn>
          </div>
            ...
            <FadeIn delay={0.2}>
              <div className={`rounded-2xl border p-5 h-full ${isDark ? 'bg-white/[0.02] border-white/[0.06]' : 'bg-white/80 border-gray-200/50'}`}>
                ...
                <div className={`text-sm space-y-2 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                  ...
                  <img src={SS_STATUSLINE} alt="statusline preview" className="mt-2 rounded-lg w-full" />
            ...
            {[
              {
                ...
                desc: 'Claude Code를 핵심 개발 도구로 활용. GPT 복붙 → VS Code AI → Claude Code로 발전하며 하네스 엔지니어링을 체득. 이 포트폴리오 페이지 자체도 Claude Code로 제작.',
                detail: 'CLAUDE.md, MCP 서버, 파일 메모리, 훅, 스킬 등 에이전트 운영 환경을 직접 설계하고 운용.',
              },
              {
        ...
        <div className="max-w-5xl mx-auto px-6">
          ...
          <FadeIn className="mb-8">
            ...
            <div className={`rounded-2xl border overflow-hidden ${isDark ? 'border-white/[0.06]' : 'border-gray-200'}`}>
              {[
                { req: 'Claude Code 등 코딩 에이전트 활용', match: 'Claude Code로 프로젝트 전체 개발 + 하네스 엔지니어링 (CLAUDE.md, MCP, 메모리, 훅, 스킬)' },
          ...
          <FadeIn delay={0.1}>
            ...
            <div className="grid md:grid-cols-2 gap-3">
              {[
                ...
                { title: '하네스 엔지니어링', desc: 'CLAUDE.md, MCP 서버, 메모리, 훅, 스킬 등 에이전트 운영 환경 설계' },
              ].map((item, i) => (
