...
class NotConnectedView(discord.ui.LayoutView):
    ...
    def __init__(self):
        ...
        box.add_item(discord.ui.TextDisplay(
            ...
            '-# 한 장에 약 70원이에요. 5달러면 100장쯤 그릴 수 있어요.'
        ))
...
class GeneratingView(discord.ui.LayoutView):
    ...
    def __init__(self, character: str, request: str):
        super().__init__(timeout=None)
        box = discord.ui.Container(accent_colour=ACCENT_WAIT)
        box.add_item(discord.ui.TextDisplay(
            f'## {character} 를 그리는 중이에요\n'
            f'> {request[:180]}\n\n'
            '**약 3분** 걸려요. 다 되면 이 메시지가 그림으로 바뀌어요.'
        ))
        self.add_item(box)
...
class ResultView(discord.ui.LayoutView):
    ...
    def __init__(self, character: str, request: str, filename: str, calls: int, est_usd: float):
        super().__init__(timeout=None)
        ...
        box.add_item(discord.ui.TextDisplay(
            f'-# 지금까지 {calls}장 · 약 ${est_usd:.2f} (본인 계정)'
        ))
        self.add_item(box)
